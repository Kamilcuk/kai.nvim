...
function M.create(buffer, start_row, start_col, end_row, end_col)
    ...
    return {buffer = buffer, extmark_id = extmark_id}
end
...
function M.set_preview_text(indicator, text)
    local extmark = vim.api.nvim_buf_get_extmark_by_id(indicator.buffer, ns_id,
                                                       indicator.extmark_id,
                                                       {details = true})
    ...
    vim.api.nvim_buf_set_extmark(indicator.buffer, ns_id, start_row, start_col,
                                 extmark_opts)
end
...
function M.set_buffer_text(indicator, text)
    ...
    local lines = vim.split(text, "\n")
    vim.api.nvim_buf_set_text(indicator.buffer, start_row, start_col, end_row,
                              end_col, lines)
end
